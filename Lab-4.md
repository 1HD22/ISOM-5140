...
### Challenge task 1: Explicitly denying the role of the "sts:AssumeRole" action

1. Switch to the browser tab where you are signed in as the root user.
2. In the IAM console, locate the *DeveloperGroupPolicy* policy and edit it.
3. Add a explicit deny policy to the "sts:AssumeRole" action right below the previously entered policy in the correct position:
```json
{
	"Action": [
		"sts:AssumeRole"
...
5. Switch back to the browser tab where you are logged in as the IAM user *devuser*.
6. Attempt to assume the *Bucket2AccessRole* again. Verify whether the action is still permitted.

**Did it work? Why not? Explain in detail.**

7. Switch to the browser tab where you are signed in as the root user. Remove the explicit deny "sts:AssumeRole" action from the *DeveloperGroupPolicy* policy.
...
2. Experiment and Answer
   - Answer the questions proposed in [Challenge task 1](#challenge-task-1-explicitly-denying-the-role-of-the-stsassumerole-action). Mainly:
      ...
      - Why are you unable to access the role even though you have set an explicit allow? Explain in detail.